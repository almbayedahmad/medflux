# PURPOSE:
#   Compute per-page and aggregate statistics for readers outputs.
#
# OUTCOME:
#   Produces normalized stats used in stage summaries and validations.
#
# INPUTS:
#   - Page records, thresholds/settings.
#
# OUTPUTS:
#   - Dict-like stats appended to stage summaries (in-memory).
#
# DEPENDENCIES:
#   - Local schemas for settings and types; standard library only.
from __future__ import annotations

"""Core business logic for per-page statistics computation in the readers stage."""

from collections import defaultdict
from typing import Any, Dict, Iterable, List, Optional, Set

from ...schemas.readers_schema_settings import get_runtime_settings
from core.preprocessing.cross_phase.helpers.main_pre_helpers_lang import collapse_doc_lang, normalise_lang as utils_normalise_lang, tokenise_langs
from core.preprocessing.cross_phase.helpers.main_pre_helpers_num import as_float, as_int

from ...schemas.readers_schema_types import PerPageStat
...
def compute_readers_lang_tokens(raw_values: Iterable[Any] | None) -> List[str]:
    """Compute language tokens from raw values."""
    tokens: List[str] = []
    ...
    for raw in iter_values or []:
        ...
        for part in str(raw).replace(",", "+").split("+"):
            ...
            if not token:
                continue
            mapped = utils_normalise_lang(token)
...
def normalize_readers_lang(raw: Any, fallback: Iterable[str] | None = None) -> str:
    """Normalize language value to standard format."""
    tokens = compute_readers_lang_tokens([raw])
    if not tokens and fallback:
        tokens = compute_readers_lang_tokens(fallback)
    if not tokens:
...
def normalize_readers_flags(flags: Iterable[Any]) -> List[str]:
    """Normalize flags to list of strings."""
    result: List[str] = []
...
def normalize_readers_source(raw: str) -> str:
    """Normalize source type to standard format."""
...
def compute_readers_page_size_payload(width: Optional[float], height: Optional[float]) -> Dict[str, float]:
    """Compute page size payload from width and height."""
...
def normalize_readers_rotation(value: Any) -> int:
    """Normalize rotation value to 0, 90, 180, or 270 degrees."""
    try:
...
def compute_readers_lang_share(
    blocks: Iterable[JsonDict],
    fallback_tokens: List[str],
) -> Dict[str, float]:
    """Compute language share from text blocks."""
    ...
    for block in blocks or []:
        ...
        if char_count <= 0:
            continue
        tokens = compute_readers_lang_tokens([block.get("lang"), block.get("lang_hint")])
        if not tokens:
            tokens = compute_readers_lang_tokens(fallback_tokens)
        if not tokens:
...
def check_readers_header_footer(blocks: Iterable[JsonDict], page_height: Optional[float]) -> bool:
    """Check if page has header/footer based on block positions."""
...
def compute_readers_text_density(chars: int, width: Optional[float], height: Optional[float]) -> float:
    """Compute text density (characters per unit area)."""
...
def compute_readers_per_page_stats(
    ...
    *,
    ...
    """Main orchestrator function to compute per-page statistics."""
    ...
    fallback_tokens = compute_readers_lang_tokens(lang_fallback)
    ...
    for entry in per_page_raw or []:
        ...
        source_normalised = normalize_readers_source(str(entry.get("source") or entry.get("decision") or "text"))
        lang_value = normalize_readers_lang(entry.get("lang"), fallback=lang_fallback)
        locale_value = normalize_readers_lang(entry.get("locale"), fallback=lang_fallback)
        flags_list = normalize_readers_flags(entry.get("flags") or [])
        ...
        rotation_value = normalize_readers_rotation(page_info.get("rotation"))
        page_size = compute_readers_page_size_payload(width, height)
        ...
        if noise_score > 1.0:
            noise_score = 1.0
        text_density = compute_readers_text_density(chars, width, height)

        blocks = blocks_lookup.get(page_number, [])
        lang_share = compute_readers_lang_share(blocks, fallback_tokens)
        if not lang_share:
            if lang_value == "de+en":
        ...
        if zone_types:
            ...
            has_header_footer = check_readers_header_footer(blocks, height)
...
lang_tokens = compute_readers_lang_tokens
normalise_lang = normalize_readers_lang
normalise_flags = normalize_readers_flags
normalise_source = normalize_readers_source
page_size_payload = compute_readers_page_size_payload
normalise_rotation = normalize_readers_rotation
compute_lang_share = compute_readers_lang_share
detect_header_footer = check_readers_header_footer
compute_text_density = compute_readers_text_density
build_per_page_stats = compute_readers_per_page_stats


__all__ = [
    "compute_readers_lang_tokens",
    "normalize_readers_lang",
    "normalize_readers_flags",
    "normalize_readers_source",
    "compute_readers_page_size_payload",
    "normalize_readers_rotation",
    "compute_readers_lang_share",
    "check_readers_header_footer",
    "compute_readers_text_density",
    "compute_readers_per_page_stats",
    # Backwards-compatible aliases
    "lang_tokens",
    "normalise_lang",
    "normalise_flags",
