# PURPOSE:
#   Detection utilities migrated from internal_helpers to the domain layer for
#   phase_00_detect_type. Implements file-type heuristics and recommendations.
#
# OUTCOME:
#   Stable helpers for classifier logic and services without relying on the
#   legacy internal_helpers package.
...
def process_detect_type_file(
    ...
    if ext in TXT_EXTENSIONS or (mime and ("text" in mime or "csv" in mime or "json" in mime)):
        recommended = {"mode": "native", "tables_mode": "off", "lang": "deu+eng"}
        return FileTypeResult(path, ext, mime, FileType.TXT, False, {"note": "text"}, confidence=0.99, recommended=recommended)
    ...
    if ext in IMAGE_EXTENSIONS or (mime and mime.startswith("image/")):
        ...
        return FileTypeResult(path, ext, mime, FileType.IMAGE, True, {"note": "image"}, confidence=0.7, recommended=recommended)

    # Default unknown / other
    return FileTypeResult(path, ext, mime, FileType.UNKNOWN, None, {"note": "unknown"}, confidence=0.5, recommended=dict(DEFAULT_RECOMMENDATION))


def process_detect_type_many(
    paths: Sequence[str],
    *,
    sample_pages: int = DEFAULT_SAMPLE_PAGES,
    topk_image_pages: int = DEFAULT_TOPK_IMAGE_PAGES,
    ...
    for path in paths:
        try:
            result = process_detect_type_file(
                path,
                sample_pages=sample_pages,
                topk_image_pages=topk_image_pages,
                img_area_thr=img_area_thr,
                text_len_thr=text_len_thr,
                words_thr=words_thr,
                blocks_thr=blocks_thr,
            )
            results.append(result)
        except Exception:
            # Continue with best-effort results; downstream can validate contracts
            results.append(
                FileTypeResult(
                    file_path=path,
                    ext=os.path.splitext(path)[1].lower(),
