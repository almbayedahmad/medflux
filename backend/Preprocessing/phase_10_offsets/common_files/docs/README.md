# Offsets Stage (phase_10_offsets)

## Purpose
Compute and validate character offsets and alignments across normalized artifacts to enable downstream highlighting and traceability.

## Workflow (v2)
- API: api.py
- CLI: cli/offsets_cli_v2.py
...
```
python -m backend.Preprocessing.phase_10_offsets.cli.offsets_cli_v2 --help
```
...
```
medflux phase-offsets --inputs path/to/file --output-root ./.artifacts/offsets
```

## Examples

- Umbrella CLI:
  `\n  medflux phase-offsets --inputs samples/Sample.txt --output-root ./.artifacts/offsets\n  `\n- Phase v2 CLI:
  `\n  python -m backend.Preprocessing.phase_10_offsets.cli.offsets_cli_v2 --inputs samples/Sample.txt --output-root ./.artifacts/offsets\n  `\n
