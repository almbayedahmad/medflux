# Segmentation Stage (phase_06_segmentation)

## Purpose
Segment normalized content into structural units (sections, paragraphs, blocks) with timing and confidence summaries.

## Workflow (v2)
- API: api.py
- CLI: cli/segmentation_cli_v2.py
...
```
python -m backend.Preprocessing.phase_06_segmentation.cli.segmentation_cli_v2 --help
```
...
```
medflux phase-segmentation --inputs path/to/file --output-root ./.artifacts/segmentation
```

## Examples

- Umbrella CLI:
  `\n  medflux phase-segmentation --inputs samples/Sample.txt --output-root ./.artifacts/segmentation\n  `\n- Phase v2 CLI:
  `\n  python -m backend.Preprocessing.phase_06_segmentation.cli.segmentation_cli_v2 --inputs samples/Sample.txt --output-root ./.artifacts/segmentation\n  `\n
