...
"""Core business logic for text block processing in the readers stage."""
...
from medflux_backend.Preprocessing.main_pre_helpers.main_pre_helpers_geom import to_bottom_left, validate_bbox
from medflux_backend.Preprocessing.main_pre_helpers.main_pre_helpers_lang import collapse_doc_lang, normalise_lang, tokenise_langs
...
def compute_readers_split_lang_candidates(*values: Any) -> List[str]:
    """Split and normalize language candidates from various input values."""
    tokens: List[str] = []
    stack = list(values)
    while stack:
        raw = stack.pop()
...
def compute_readers_resolve_lang(raw: Any, fallback: Any = None, text: str | None = None) -> str:
    """Resolve language from raw value, fallback, or text analysis."""
...
def compute_readers_ensure_float_list(values: Any) -> List[float]:
    """Ensure values are converted to list of floats."""
...
def normalize_readers_text_lines(value: Any, fallback: str) -> List[str]:
    """Normalize text lines from various input formats."""
...
def check_readers_bool_flag(value: Any, default: bool = False) -> bool:
    """Check boolean flag from various input types."""
...
def compute_readers_lang_confidence(lang: str) -> float:
    """Compute language confidence score."""
    if lang == "de+en":
        return 0.7
...
def compute_readers_paragraph_style(is_heading: bool, is_list: bool) -> str:
    """Compute paragraph style from heading and list flags."""
    if is_heading:
        return "heading"
    if is_list:
        return "list"
...
def compute_readers_block_type(
    ...
    """Compute block type based on properties and position."""
...
def compute_readers_text_blocks(
    ...
    """Main orchestrator function to compute text blocks from JSONL file."""
...
__all__ = [
    "compute_readers_split_lang_candidates",
    "compute_readers_resolve_lang",
    "compute_readers_ensure_float_list",
    "normalize_readers_text_lines",
    "check_readers_bool_flag",
    "compute_readers_lang_confidence",
    "compute_readers_paragraph_style",
    "compute_readers_token_count",
    "compute_readers_numbering_marker",
    "compute_readers_block_type",
    "compute_readers_text_blocks",
]
