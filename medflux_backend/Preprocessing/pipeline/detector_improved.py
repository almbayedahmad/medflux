...
"""Improved heuristics for determining document processing mode."""
import mimetypes
import os
...
DEFAULT_RECO = {"mode": "native", "dpi": 200, "psm": 6, "tables_mode": "light"}
...
DOCX_EXTS = {".docx"}
TXT_EXTS = {".txt", ".log", ".md", ".csv", ".tsv", ".json", ".yaml", ".yml", ".ini", ".cfg", ".conf"}
...
class FileTypeResult:
    ...
    def __init__(
        ...
        self.ocr_recommended = ocr_recommended
...
def _analyze_page(page, text_len_thr: int, words_thr: int, blocks_thr: int, img_area_thr: float) -> Dict[str, Any]:
...
def detect_file_type_improved(
    ...
    if ext == ".pdf" or (mime and mime.endswith("pdf")):
        ...
        if scanned is True:
            ...
            recommended = {"mode": "ocr", "dpi": 320, "psm": 6, "tables_mode": "light"}
            ocr_rec = True
        elif mixed:
            file_type = PDF_MIXED
            recommended = {"mode": "mixed", "dpi": 300, "psm": 6, "tables_mode": "light"}
            ocr_rec = True
        else:
            file_type = PDF_TEXT
        ...
        return FileTypeResult(path, ext, mime, file_type, ocr_rec, details, confidence=confidence, recommended=recommended)

    if ext in DOCX_EXTS or (mime == "application/vnd.openxmlformats-officedocument.wordprocessingml.document"):
        recommended = {"mode": "native", "tables_mode": "light"}
        return FileTypeResult(path, ext, mime, DOCX, False, {"note": "docx"}, confidence=0.95, recommended=recommended)

    if ext in TXT_EXTS or (mime and (mime.startswith("text/") or "json" in mime or "yaml" in mime or "csv" in mime)):
        recommended = {"mode": "native", "tables_mode": "off"}
        return FileTypeResult(path, ext, mime, TXT, False, {"note": "text"}, confidence=0.95, recommended=recommended)

    if ext in IMAGE_EXTS or (mime and mime.startswith("image/")):
        recommended = {"mode": "ocr", "dpi": 320, "psm": 6, "tables_mode": "light"}
        return FileTypeResult(path, ext, mime, IMAGE, True, {"note": "image"}, confidence=0.95, recommended=recommended)

    return FileTypeResult(path, ext, mime, UNKNOWN, False, {"note": "unknown"}, confidence=0.3, recommended={"mode": "mixed", "tables_mode": "light"})


def detect_many_improved(paths: List[str], **kwargs) -> List[FileTypeResult]:
    results: List[FileTypeResult] = []
    for path in paths:
        try:
            results.append(detect_file_type_improved(path, **kwargs))
